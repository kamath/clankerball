/* ============================================================
   players.ts — team & player configuration + random generator
   Ported from players.js.
   ============================================================ */
import type { GameConfig, PlayerConfig } from "@repo/shared";
...
/* ----------------------------------------------------------------
   ...
   ---------------------------------------------------------------- */
export const LOCKDOWN: PlayerConfig[] = [
...
export const FIREPOWER: PlayerConfig[] = [
...
interface Archetype {
  pos: string;
  h: [number, number]; w: [number, number];
  iq: [number, number]; three: [number, number]; mid: [number, number];
  layup: [number, number]; dunk: [number, number];
  bh: [number, number]; pas: [number, number];
  spd: [number, number]; acc: [number, number]; str: [number, number]; vrt: [number, number];
  pd: [number, number]; idf: [number, number]; stl: [number, number]; blk: [number, number]; reb: [number, number];
  tShoot: [number, number]; tThree: [number, number]; tDrive: [number, number]; tPass: [number, number];
  tHelp: [number, number]; tCrash: [number, number]; tGamble: [number, number];
}
...
const ri = (r: [number, number]) => r[0] + Math.floor(Math.random() * (r[1] - r[0] + 1));

export function randomRoster(usedNames?: Set<string>): PlayerConfig[] {
...
export const DEFAULT_CONFIG: GameConfig = {
  ...
  teamA: { name: "Lockdown Five", abbr: "LCK", color: "#21b0b8", players: LOCKDOWN },
  teamB: { name: "Firepower Five", abbr: "FPW", color: "#ef5b2b", players: FIREPOWER },
};
