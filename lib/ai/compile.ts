...
import { z } from "zod";
import { sanitizePlan, type TeamPlan } from "@/lib/plan";
import type { Tendencies } from "@/lib/types";

const MODEL = process.env.AI_GATEWAY_MODEL || "minimax/minimax-m3";

/** Serializable roster snapshot the client sends for compilation. */
...
export interface CompileRequest {
  ...
  roster: ScoutPlayer[];
  opponentName: string;
  opponentRoster: ScoutPlayer[];
  /** where these instructions apply, to shade the prompt */
  context: "lab-offense" | "lab-defense" | "game";
}
