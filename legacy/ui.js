'use strict';
/* ============================================================
   ui.js — canvas renderer for the court and players
   ============================================================ */
(function (global) {

const { COURT } = global.BBEngine;
...
class Renderer {
  ...
  px(x) { return PAD + x * SCALE; }
  py(y) { return PAD + y * SCALE; }
  ...
  buildCourt(colors) {
    ...
    const w = COURT.W * SCALE, h = COURT.H * SCALE;
    ...
    const sheen = c.createRadialGradient(
      x0 + w / 2, y0 + h / 2, 40, x0 + w / 2, y0 + h / 2, w / 1.6);
  ...
  draw(game, dt) {
    ...
    for (let ti = 0; ti < 2; ti++) {
      ...
      for (const p of team.players) {
        const x = this.px(p.pos.x), y = this.py(p.pos.y);
    ...
    // ball
    const b = game.ball;
    const bx = this.px(b.pos.x), by = this.py(b.pos.y);
    const air = b.air || 0;
    ...
    if (game.over) {
      ...
      c.fillText(`${a.name} ${a.score} — ${bteam.score} ${bteam.name}`, this.W / 2, this.H / 2 + 26);
    }
  }
}

global.BBUI = { Renderer };
